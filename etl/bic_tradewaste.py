...
from utils import url, fields, quick_clean, get_hnum, get_sname, get_geocode
...
bic_tradewaste = Flow(
        ...
        add_computed_field([dict(target=dict(name = 'address', type = 'string'),
                                ...
                                dict(target=dict(name = 'geo', type = 'object'),
                                        operation=lambda row: get_geocode(row['hnum'], row['sname'], '', row['zipcode'])
                                        )
                                ]),
