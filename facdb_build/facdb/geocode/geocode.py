...
def geocode(input):
    ...
    try:
        ...
        geo.update(
            uid=uid, hnum=hnum, sname=sname, address=address, address_c=address_c
        )
        return geo
...
def parse_output(geo):
    return dict(
        xcoord=geo.get("SPATIAL X-Y COORDINATES OF ADDRESS", {}).get(
            "X Coordinate", ""
        ),
        ycoord=geo.get("SPATIAL X-Y COORDINATES OF ADDRESS", {}).get(
            "Y Coordinate", ""
        ),
...
if __name__ == "__main__":
    ...
    df = pd.read_sql(
        "select address, borocode, uid from facilities where geom is null and borocode is not null;",
        engine,
    )
