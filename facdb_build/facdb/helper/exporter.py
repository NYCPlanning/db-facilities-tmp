...
def exporter_classic(
    df, table_name, con=build_engine, chunksize=10000, to_geom=True, SRID=4326
):
    df = df.replace(r"^\s+$", None, regex=True)

    df.to_sql(
        table_name, con=con, if_exists="replace", index=False, chunksize=chunksize
    )

    # # Change to target DDL
    for i in df.columns:
        con.connect().execute(
            f'ALTER TABLE {table_name} ALTER COLUMN "{i}" TYPE text USING ("{i}"::text);'
        )
        con.connect().execute(
            f'UPDATE {table_name} SET "{i}"=NULLIF(TRIM("{i}"), \'\');'
        )
        con.dispose()
